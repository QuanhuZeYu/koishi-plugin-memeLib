...
async function createFrame(inputImg, handImg, frame, option = {
    ...
    try {
        // 缩放输入图片并合成到背景图片
        const resizedInputImg = await (0, index_1.default)(inputImg)
            .resize(frame.width, frame.height)
            .toBuffer();
        return await (0, index_1.default)(handImg)
            .composite([
            {
        ...
        const bo = index_2.default.imageTools.isPng(inputImg);
        const bo2 = index_2.default.imageTools.isJpg(inputImg);
        const ho = index_2.default.imageTools.isPng(handImg);
...
async function loadHandImages(frames) {
    ...
    (0, index_1.default)(node_path_1.default.join(__dirname, `images/${index}.png`))
        .toBuffer()
        .catch((err) => {
        // 捕获并打印任何读取图像时发生的错误
...
async function genPetpetGif(inputImg, isGif = false) {
    if (!isGif) {
        ...
        try {
            ...
            let inputCounts = 0;
            while (_b === false) {
                oldInputs = await index_2.default.gifTools.extractGifFramesFromBuffer(inputImg, undefined, 15);
                inputCounts = oldInputs.length;
                _b = index_2.default.imageTools.isPng(oldInputs[inputCounts - 1]);
